...
import { Bookmark, BookmarkCheck, Currency, Layers, Loader2, Network } from "lucide-react"; // Removed 'Briefcase'
      ...
      <Card>
        <div className="w-full h-full p-4 flex flex-col items-start justify-start gap-y-4">
          ...
          <Box>
            ...
            <Button variant="ghost" size="icon" onClick={onClickSaveJob}>
              {isBookmarkloading ? (
                ...
                <SavedUsersicon
                  className={cn("w-4 h-4", isSavedUser ? "text-emerald-500" : "text-muted-foreground")}
                />
              )}
            </Button>
          </Box>
          ...
          <Box className="items-center justify-start gap-x-4">
            <div className="w-12 h-12 min-w-12 m-h-12 border p-2 rounded-md relative flex items-center justify-center overflow-hidden">
              {company?.logo && <Image alt={company?.name} src={company?.logo} width={40} height={40} />}
            </div>
          ...
          {job.short_description && (
            <CardDescription className="text-xs">
              {truncate(job.short_description, { length: 180, omission: "..." })}
            </CardDescription>
          )}
          ...
          <Box className="gap-2 mt-auto">
            <Link href={`/search/${job.id}`} className="w-full">
              <Button className="w-full border-blue-500 text-blue-500 hover:bg-transparent hover:text-blue-500" variant="outline">
                Details
              </Button>
            </Link>

            <Button
              className="w-full text-white hover:bg-blue-500 bg-blue-800/90 hover:text-white"
              variant="outline"
